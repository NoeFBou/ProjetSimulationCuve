...
public class PanelCapacites extends JPanel implements ActionListener {
	...
	public PanelCapacites(ControleurGUI ctrl, FrameCuvesGUI frame) {
		this.ctrl = ctrl;
		this.frame = frame;
		...
		Object actId = this.btnValider.getInputMap().get(KeyStroke.getKeyStroke(KeyEvent.VK_SPACE, 0));
		this.btnValider.getInputMap().put(KeyStroke.getKeyStroke(KeyEvent.VK_ENTER, 0), actId);
		// rebind "space" action to "enter" for key released event
		actId = this.btnValider.getInputMap().get(KeyStroke.getKeyStroke(KeyEvent.VK_SPACE, 0, true));
		this.btnValider.getInputMap().put(KeyStroke.getKeyStroke(KeyEvent.VK_ENTER, 0, true), actId);
		// end rebind
